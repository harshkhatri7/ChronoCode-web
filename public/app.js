...
function connectWS() {
  ...
  const wsToken = localStorage.getItem('cc_token') || '';
  ...
  socket = new WebSocket(`${protocol}//${window.location.host}?token=${encodeURIComponent(wsToken)}&email=${emailParam}&platform=${platformParam}`);
...
if (window.electronAPI) {
  ...
  // If not running in Electron, hide the window controls
  const winControls = document.querySelector('.window-controls');
  if (winControls) winControls.style.display = 'none';
}

// ─────────────────────────────────────────────
// AUTO UPDATE CHECK (Electron only)
// ─────────────────────────────────────────────
if (window.electronAPI && window.electronAPI.checkUpdate) {
  setTimeout(async () => {
    try {
      const update = await window.electronAPI.checkUpdate();
      if (update && update.needsUpdate) {
        showToast(`Update available: v${update.latestVersion} (current: v${update.currentVersion}). ${update.notes || ''}`, 'info');
        const updateToast = document.getElementById('toastContainer');
        if (updateToast) {
          const btn = document.createElement('button');
          btn.textContent = 'Download Update';
          btn.style.cssText = 'margin-top:8px;padding:6px 14px;background:var(--accent);color:#fff;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;font-family:Inter,sans-serif;';
          btn.onclick = () => window.electronAPI.openDownload(update.downloadUrl);
          const lastToast = updateToast.querySelector('.toast');
          if (lastToast) lastToast.appendChild(btn);
        }
      }
    } catch (_) {}
  }, 5000);
